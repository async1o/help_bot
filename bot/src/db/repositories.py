    ...
    async def add_user(self, user: UserSchema):
        async with async_session_maker() as session:
            ...
            try:
                ...
                logger.warning(
                    "User %s already exists (IntegrityError): %s", user.user_id, e
                )
    ...
    async def add_start_admins(self):
        for admin_id in settings.ADMINS.split(","):
            await UserRepository().add_user(
                UserSchema(user_id=admin_id, is_admin=True, is_operator=False)
            )
        logger.info(msg="Admins added to DB")

    async def add_start_operators(self):
        for operator_id in settings.OPERATORS.split(","):
            await UserRepository().add_user(
                UserSchema(user_id=operator_id, is_admin=False, is_operator=True)
            )
        logger.info(msg="Operators added to DB")
...
class DialogRepository:
    ...
    async def add_dialog(self, dialog: DialogSchema) -> None:
        ...
        async with async_session_maker() as session:
            ...
            await session.commit()
            logger.info(
                "Dialog added: operator=%s, sender=%s",
                dialog.operator_id,
                dialog.sender_id,
            )
    ...
    async def remove_by_operator(self, operator_id: str) -> str | None:
        ...
        async with async_session_maker() as session:
            stmt = select(DialogModel.sender_id).where(
                DialogModel.operator_id == operator_id
            )
            result = await session.execute(stmt)
    ...
    async def remove_by_user(self, sender_id: str) -> str | None:
        ...
        async with async_session_maker() as session:
            stmt = select(DialogModel.operator_id).where(
                DialogModel.sender_id == sender_id
            )
            result = await session.execute(stmt)
