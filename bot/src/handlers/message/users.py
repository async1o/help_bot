...
async def start(message: Message):
    ...
    greeting = f"Здравствуйте, {display_name}!\n Для обращения в службу поддержки напишите /sos"
...
async def confirm_request(message: Message, state: FSMContext):
    ...
    await message.answer(
        text="Убедитесь, что всё правильно", reply_markup=confirmation_kb()
    )
...
async def apply_answer(message: Message, state: FSMContext):
    try:
        ...
        if request_id:
            await message.answer(
                text="Запрос отправлен!", reply_markup=ReplyKeyboardRemove()
            )
        else:
            await message.answer(
        ...
        await message.answer(
            text="Что-то пошло не так", reply_markup=ReplyKeyboardRemove()
        )
    finally:
        await state.clear()
